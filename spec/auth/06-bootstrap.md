...
3. **Something has to get back in.** Every credential lost, the last Superuser
...
## 2. It is always Superuser-equivalent, and that must be stated

The token is **outside the tier system**. It is not an identity with `tier:
Superuser`; it is the credential that exists before identities do, and it can do
everything.
...
- **Demoting yourself changes nothing if you also hold the env token.** An
  `Superuser` who sets their identity to `Member` and still has `MCORCH_API_TOKEN` is
  still a Superuser, by a different door.
- **Host read access is Superuser access.** Threat model item 5 already says the
...
identity table empty
        │
        ├── bearer MCORCH_API_TOKEN ──► Superuser-equivalent, always
        │
...
lost, and a lost Superuser credential is one nobody holds and nothing revokes.

## 4. Recovery

| Situation | Way back |
|---|---|
| An operator lost their credential | An `Superuser` rotates it — [05-api.md](05-api.md) §2 |
| The last Superuser was disabled | Refused by `LAST_SUPERUSER` before it can happen; if it happened anyway, the env token |
