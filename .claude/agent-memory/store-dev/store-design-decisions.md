...
**Deadline-bearing durations are clamped on the way out, and the row is never rewritten.**
**Why:** the thirty-first audit ruled that `stopGracePeriod`, `drain.saveTimeout` and `backends.drain.sealTimeout` become transport deadlines bounded only by their YAML reader, so any row that skipped a reader parks a reconcile worker. A `require` in the spec types was ruled out *twice* — `Reconciler.rejectDefinition` records `PERMANENT` without exempting `terminating`, so an `init` makes a populated server undeletable. Refusing the row via the `Unreadable` path was rejected for the same shape of reason: an undecodable definition is one the loop cannot drain, stop or purge, so the container outlives the orchestrator. Clamping is safe only because of *what* is shortened — a wait that can withhold a confirmation, or a stop already gated on a confirmed save.
**How to apply:** the rule lives in `:schema`'s `SpecBounds` (public, borrowing every ceiling from the readers' own constants), not in `:store`, because a second implementation owes the same guarantee — `StoreConformanceSuite` asserts it and `InMemoryStore` applies it at its read methods. Ceilings only, never floors: raising `saveTimeout` raises the minimum the grace period must clear and inverts the validated pair, which is a container SIGKILLed mid-save. `SpecBounds.init` `require`s `MAX_STOP_GRACE_PERIOD >= MAX_TIMEOUT + MIN_STOP_GRACE_MARGIN` so the two borrowed constants cannot drift into an inversion. `startupTimeout` and `playerTransferTimeout` were examined and cleared as wall-clock comparisons — there are tests pinning that they stay unbounded, so do not "make it consistent".

**No migration was added for out-of-range rows, and that is the decision rather than an omission.**
**Why:** the bound is a read-path rule, so an old row gets it whatever version wrote it — a migration would buy nothing. Capping the value *on disk* would discard the operator's declared number, which CLAUDE.md forbids, and SQLite cannot guard a value living inside the `spec_doc` TEXT blob with a trigger the way V4 guards a column. A version number spent on a log line can never be reused.
**How to apply:** the upgrade path is covered by a `MigrationTest` case instead — written at version 1, migrated through all five, asserted still on disk byte-for-byte and served bounded, with the tombstoned one still deletable and purgeable. If someone proposes normalising these values in a migration, that test is the argument.
